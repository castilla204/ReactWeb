...
const allTabs = [
  ...
  { id: 'relocate' as TabType, label: 'Cambiar país', icon: Plane, expertOnly: true },
...
export const AccountSettingsModal: React.FC<AccountSettingsModalProps> = ({
  ...
  // ?? Mantener el input de nombre en sync con el usuario (al abrir el modal o tras refresh).
  useEffect(() => {
  ...
  const renderContent = () => (
    <>
      ...
      {activeTab === 'profile' && (
        <div className="space-y-6">
          ...
          <div className="flex items-center gap-4">
            {/* ??? Avatar: foto si existe, si no iniciales (registro por email sin foto) */}
            {avatarUrl ? (
              <img
                src={avatarUrl}
                alt="Foto de perfil"
          ...
          {/* ??? Recorte de la foto antes de subir (reutiliza el flujo del experto) */}
                ...
                <div className="flex items-center justify-between p-4 border border-border rounded-xl">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground">Próximamente podrás cambiarla desde aquí</p>
                    </div>
                  </div>
          ...
          <div className="flex flex-col gap-3 p-4 border border-border rounded-xl sm:flex-row sm:items-center sm:justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground">Próximamente podrás controlar quién ve tu perfil</p>
              </div>
            </div>
            <select
              disabled
              defaultValue="Público"
              aria-label="Visibilidad del perfil (próximamente)"
      ...
      {/* ??? Round 28 MUD-F: Tab Cambiar país (solo expertos). El detalle completo
          (qué se conserva/desactiva, pasos, confirmación) vive en ExpertRelocationWizard —
          aquí solo se explica el motivo y se abre el mismo wizard, sin duplicar su copy. */}
      {activeTab === 'relocate' && isExpert && (
        <div className="space-y-5">
          <div className="p-6 bg-muted/50 border border-border rounded-xl">
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1">
                <h4 className="text-base font-semibold text-foreground">¿Te has mudado a otro país?</h4>
                <p className="text-sm text-muted-foreground mt-1">
                  Stripe Connect no permite cambiar el país de tu cuenta de cobros. Este asistente cierra tu cuenta Stripe actual y te prepara para hacer un onboarding nuevo en tu país de residencia.
                </p>
              </div>
            </div>
          </div>
          ...
          <Button
            onClick={() => {
              // Este modal es global (se abre desde cualquier página vía "Mi cuenta"),
              ...
              // escuchaba — si el experto abría "Configuración" fuera del panel (p. ej.
              // desde la home), ExpertPanelPage no estaba montado y el evento no hacía
              // nada. Navegar con ?relocate=1 funciona desde cualquier página: el panel
              // lee el parámetro al montar y abre el wizard él mismo.
              onClose();
        ...
        <Dialog open={isOpen} onOpenChange={handleDialogOpenChange} modal={true}>
          <DialogContent 
            ...
            onPointerDownOutside={(e) => {
              // ??? Round 28 MUD-T: dejar pasar clicks del wizard de mudanza (portal en body).
  ...
  return (
    <>
      ...
      {/* El wizard de "Cambiar país" NO se monta aquí — solo vive dentro de
          ExpertPanelPage. Este modal es global y puede abrirse desde cualquier página,
          así que su botón navega a /expert?relocate=1 en vez de dispatchar un evento
          (un evento global no sirve si ExpertPanelPage no está montado todavía). */}
    </>
  );
};
  ...
  return (
    <div className="flex items-start justify-between gap-3 p-3">
      ...
      <div className="flex flex-shrink-0 flex-col items-end">
        ...
        <span className="mt-0.5 text-kicker text-muted-foreground">{statusLabel}</span>
      </div>
    </div>
  );
};
