...
// import { useCategories } from '../contexts/CategoryContext'; // ❌ YA NO NECESARIO - viene en details-complete
...
// interface Category { // ❌ YA NO NECESARIO - viene en details-complete
//     id: number;
//     name: string;
// }
...
export default function SearchDetails({ isAdmin, onBack }: SearchDetailsProps) {
    ...
    // const { categories } = useCategories(); // ❌ YA NO NECESARIO - viene en details-complete
    const { user } = useAuth();
    ...
    const {
        ...
        moneyDistribution,
        category,
        review, // ✅ NUEVO: Review del endpoint details-complete
        conversations,
        appointment,
        deliverables,
        disputes,
    ...
    // Debug temporal para verificar los datos del experto - MOVIDO DESPUÉS DE isExpert
    
    // Obtener el expertId de la disputa (puede ser diferente al de la b�squeda)
    const disputeExpertId = Number(disputes[0]?.expert?.id ?? search?.searchHire?.expert?.id ?? 0); // Usar datos optimizados
    ...
    // Debug específico para isExpert
    console.log('[SearchDetails] isExpert debug:', {
        userId,
        expertId,
        expertIdFromExpert: search?.searchHire?.expert?.id,
        isExpertFromExpertId: userId === expertId,
        isExpertFromExpertObject: search?.searchHire?.expert?.id && userId === Number(search.searchHire.expert.id),
        finalIsExpert: isExpert
    });
    
    // Debug completo para verificar los datos del experto
    console.log('[SearchDetails] Expert data debug:', {
        ...
        // Debug específico para el caso actual
        isExpertFromExpertId: userId === expertId,
        isExpertFromExpertObject: search?.searchHire?.expert?.id && userId === Number(search.searchHire.expert.id),
        expertIdNumber: Number(search?.searchHire?.expert?.id),
    ...
    const hasReviewed = !!review; // Si existe review, ya se ha puesto una reseña
    const canReview =
    ...
    // Usar el valor del backend si está disponible, sino calcularlo
    ...
    // Debug para entender por qué canExpertRespond es false
    console.log('[SearchDetails] canExpertRespond debug:', {
        canExpertRespond,
        isDisputeExpert,
        userId,
        disputeExpertId,
        disputeStatus: disputes[0]?.status,
        hasExpertResponse: !!disputes[0]?.expertResponse,
        searchHireStatus: search?.searchHire?.status,
        disputeData: disputes[0],
        canExpertRespondFromBackend: disputes[0]?.canExpertRespond
    });
    ...
    useEffect(() => {
        ...
        console.log('[SearchDetails] Deliverables query status:', {
            isLoading: false, // ✅ Ya no hay query separada - viene en details-complete
            isError: false,
            error: null,
        });
        ...
        deliverables,
        false, // deliverablesQuery?.isLoading - ya no hay query separada
        false  // deliverablesQuery?.isError - ya no hay query separada
    ]);
        ...
        return (
            <div className={`border rounded-lg p-6 ${getResolutionColor()}`}>
                <div className="flex items-center gap-3 mb-4">
                    {getResolutionIcon()}
                    <h3 className="text-lg font-semibold text-gray-800">{getResolutionTitle()}</h3>
                </div>
                ...
                <div className="space-y-4">
                    ...
                    <div>
                        ...
                        <p className="text-gray-800 bg-white p-3 rounded-lg border">{dispute.reason}</p>
                    ...
                    {dispute.expertResponse && (
                        <div>
                            ...
                            <p className="text-gray-800 bg-white p-3 rounded-lg border">{dispute.expertResponse}</p>
                    ...
                    {dispute.resolutionComments && (
                        <div>
                            ...
                            <p className="text-gray-800 bg-white p-3 rounded-lg border">{dispute.resolutionComments}</p>
                        </div>
                    )}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div className="bg-white p-4 rounded-lg border">
                            ...
                            <p className="text-lg font-semibold text-gray-800">{formatCurrency(dispute.searchHire.amount)}</p>
                        </div>
                        ...
                        <div className="bg-white p-4 rounded-lg border">
                            ...
                            <p className="text-sm text-gray-800">{formatDate(dispute.createdAt)}</p>
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div className="bg-white p-4 rounded-lg border">
                            ...
                            <p className="text-sm text-gray-800">{dispute.client.name}</p>
                        </div>
                        ...
                        {dispute.expert && (
                            <div className="bg-white p-4 rounded-lg border">
                                ...
                                <p className="text-sm text-gray-800">{dispute.expert.name}</p>
                            </div>
                        )}
                    </div>
    ...
    return (
        <div className="bg-gradient-to-br from-gray-50 via-blue-50/30 to-indigo-50/50 text-black lg:h-screen lg:min-h-screen flex flex-col">
                    ...
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2 min-w-0 flex-1">
                            ...
                            <div className="min-w-0 flex-1">
                                ...
                                <h1 className="text-sm sm:text-xl font-semibold text-gray-800 truncate leading-tight">
            ...
            <div className="flex flex-col lg:flex-row lg:h-[calc(100vh-100px)] lg:max-w-none lg:mx-0 lg:gap-6 lg:p-6">
                    ...
                    <div className="lg:flex-1 lg:w-[70%] xl:w-[75%] flex flex-col lg:h-full bg-white border border-gray-200 overflow-hidden">
                                ...
                                <div className="flex items-center gap-3 flex-1">
                                    <div className="relative">
                                        {(expertInfo?.profilePictureUrl) ? ( 
                                            ...
                                            <div className="w-10 h-10 bg-gray-600 rounded-full flex items-center justify-center text-gray-800 text-sm font-medium">
                        ...
                        <div className="lg:bg-white lg:rounded-2xl lg:shadow-xl lg:border lg:border-gray-200/50 lg:mx-6 flex flex-col lg:flex-1 lg:flex-none lg:h-auto lg:backdrop-blur-sm">
                            ...
                            <div className="lg:flex-1 lg:px-4 lg:py-2 lg:overflow-hidden">
                                ...
                                {/* ✅ Citas eliminadas del tab chat - ahora solo en tab details */}

                            {/* Bot�n para proponer cita inicial - Mobile */}
                                {/* ✅ COMENTADO: Citas eliminadas del tab chat */}
                                {false && activeTab === 'chat' && needsAppointment && !appointment && search?.searchHire && isClient && canProposeAppointment() && (
                                <div className="fixed inset-x-0 bottom-0 z-40 p-3 bg-white border-t border-gray-200">
                                    <div className="flex items-center justify-between">
                                        <div className="flex items-center gap-2">
                                            <Calendar className="w-3 h-3 text-gray-600" />
                                            <div>
                                                <h3 className="text-xs font-medium text-gray-800">�Necesitas programar una cita?</h3>
                                        ...
                                        <button
                                            ...
                                            className="bg-gray-600 text-gray-800 px-3 py-1.5 rounded text-xs hover:bg-gray-700 transition-colors flex items-center gap-1"
                                        >
                                    ...
                                    <div className="lg:hidden h-full overflow-y-auto">
                                        ...
                                        <div className="p-3 space-y-3">
                                            ...
                                            <div className="bg-white rounded-xl border border-gray-200 overflow-hidden shadow-sm hover:shadow-md transition-shadow duration-200">
                                                <div className="flex">
                                                    ...
                                                    <div className="flex-1 p-2 relative">
                                                        ...
                                                        <h3 className="font-semibold text-gray-800 mb-0.5 leading-tight pr-16 text-sm">
                                                            {search?.title}
                                                        </h3>
                                            ...
                                            <div className="bg-white rounded-lg border border-gray-200 p-3 shadow-sm">
                                                <h4 className="font-medium text-gray-800 mb-2 flex items-center gap-1.5 text-sm">
                                            ...
                                            <div className="mt-6 pt-4 border-t border-gray-200">
                                                <div className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-xl p-4 border border-blue-200">
                                                    ...
                                                    {false ? ( // Ya no hay loading separado - viene en details-complete
                                                        <div className="flex items-center justify-center gap-3 py-6">
                                                            <div className="animate-spin rounded-full h-5 w-5 border-b-2 border-blue-600"></div>
                                                            <p className="text-slate-600 font-medium text-sm">Cargando informes...</p>
                                                        </div>
                                                    ) : false ? ( // Ya no hay error separado - viene en details-complete
                                                        <div className="bg-red-50 border border-red-200 rounded-lg p-3">
                                                            ...
                                                            <button 
                                                                onClick={() => window.location.reload()} // ✅ Recargar página para obtener datos actualizados
                                                                className="px-3 py-1.5 bg-red-600 text-gray-800 text-xs rounded-lg hover:bg-red-700 transition-colors"
                                                            >
                                                                Reintentar
                                                            </button>
                                                        </div>
                                                                            ...
                                                                            <div className="flex items-center gap-2">
                                                                                {deliverable.url.endsWith('.mp4') ? (
                                                                                    ...
                                                                                    <a 
                                                                                        ...
                                                                                        className="w-full px-3 py-2 bg-blue-600 text-gray-800 text-xs rounded-lg hover:bg-blue-700 transition-colors flex items-center justify-center gap-2"
                                                                                    >
                                                            ...
                                                            <div className="space-y-3">
                                                                ...
                                                                <button
                                                                    ...
                                                                    className={`w-full py-2.5 text-xs rounded-lg flex items-center justify-center gap-2 font-medium ${selectedDeliverableFiles.length === 0 || isUploadingDeliverable
                                                                        ...
                                                                        : 'bg-blue-600 hover:bg-blue-700 text-gray-800'
                                                                        }`}
                                            ...
                                            {needsAppointment && (
                                                <div className="space-y-3">
                                                    {appointment ? ( // appointmentQuery.data - datos del hook optimizado
                                                        <>
                                                            {console.log('[SearchDetails] Appointment debug:', {
                                                                appointmentStatus: appointment.status,
                                                                isClient,
                                                                isExpert,
                                                                userId,
                                                                clientId,
                                                                expertId,
                                                                userRole: isClient ? 'client' : (isExpert ? 'expert' : 'client'),
                                                                appointmentId: appointment.id,
                                                                proposedDate: appointment.proposedDate,
                                                                proposedTime: appointment.proposedTime,
                                                                shouldShowButtons: isExpert && appointment.status === 'appointment_proposed'
                                                            })}
                                                            <AppointmentStatus
                                                        ...
                                                        <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                                                            <div className="flex items-center space-x-3">
                                                                ...
                                                                <button
                                                                    ...
                                                                    className="bg-blue-600 text-gray-800 px-3 py-2 rounded-md hover:bg-blue-700 transition-colors text-xs"
                                                                >
                                                                    Proponer Cita
                                                                </button>
                                                            </div>
                                                        ...
                                                        <div className="bg-gray-50 border border-gray-200 rounded-lg p-4">
                                                            <div className="flex items-center space-x-3">
                                                                ...
                                                                <div className="flex-1">
                                                                    <h3 className="text-sm font-medium text-gray-800">Esperando propuesta de cita</h3>
                                ...
                                <div className="hidden lg:block bg-white flex-1 flex flex-col border border-gray-200 overflow-hidden">
                                    ...
                                    <div className="bg-white border-b border-gray-200 p-4">
                                        <div className="flex items-center justify-between">
                                            <div className="flex items-center gap-4">
                                                ...
                                                <div>
                                                    <h2 className="text-xl font-bold text-gray-800">Conversaci�n</h2>
                                            ...
                                            {needsAppointment && (
                                                <div className="flex items-center gap-3">
                                                    {appointment ? ( // appointmentQuery.data - datos del hook optimizado
                                                        <div className="bg-green-500 text-gray-800 px-4 py-2 rounded-full shadow-lg">
                                                            <span className="text-sm font-semibold">? Cita Programada</span>
                                                        </div>
                                                    ) : (
                                                        <div className="bg-gradient-to-r from-orange-500 to-red-500 text-gray-800 px-4 py-2 rounded-full shadow-lg">
                                    ...
                                    {needsAppointment && !appointment && isClient && canProposeAppointment() && ( // appointmentQuery.data - datos del hook optimizado
                                        <div className="bg-gray-600 text-gray-800 p-4">
                                            <div className="flex items-center justify-between">
                                                <div className="flex items-center gap-4">
                                                    ...
                                                    <div>
                                                        <h4 className="font-semibold text-gray-800 text-lg">�Necesitas programar una cita?</h4>
                ...
                <div className="hidden lg:block lg:w-[30%] xl:w-[25%] bg-white border border-gray-200 overflow-y-auto h-full">
                    ...
                    <div className="p-4 border-b border-gray-200 bg-gray-50">
                                ...
                                {false ? ( // serviceInfo - datos del hook optimizado (no incluye imageUrls)
                                    <div className="relative w-12 h-12 overflow-hidden rounded-lg">
                                        ...
                                        {false && ( // No hay múltiples imágenes en el DTO optimizado
                                            <div className="absolute -bottom-1 -right-1 bg-black/75 text-gray-800 text-xs px-1 py-0.5 rounded text-[10px] leading-none">
                                                +0
                                            </div>
                                        )}
                                    </div>
                        ...
                        <div className="space-y-3 p-4 bg-gray-50 border border-gray-200">
                            <div className="grid grid-cols-2 gap-2 lg:gap-3 text-xs">
                                <div>
                                    ...
                                    <div className="flex items-center gap-2">
                                        <div className="w-6 h-6 bg-gray-600 rounded-full flex items-center justify-center text-gray-800 text-xs font-medium">
                                ...
                                <div>
                                    ...
                                    <div className="flex items-center gap-1">
                                        ...
                                        <span className="font-medium text-gray-800">Mi proyecto</span>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="grid grid-cols-2 gap-2 lg:gap-3 text-xs">
                                ...
                                <div>
                                    ...
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div className={`w-10 h-10 bg-gray-600 rounded-full flex items-center justify-center text-gray-800 text-sm font-medium ${(search?.searchHire?.expert?.profilePictureUrl) ? 'hidden' : ''}`}>
                                            {(search?.searchHire?.expert?.name || 'E').charAt(0)}
                                        </div>
                                        <div className="flex flex-col">
                                            <span className="font-medium text-gray-800">{search?.searchHire?.expert?.name || 'Experto'}</span>
                                        </div>
                                    </div>
                            </div>
                            </div>
                        ...
                        {(isAdmin || isExpert) && (
                            <button
                                ...
                                className="w-full mt-4 lg:mt-6 bg-gradient-to-r from-gray-900 to-gray-800 text-gray-800 py-2.5 lg:py-3 rounded-lg lg:rounded-xl hover:from-gray-800 hover:to-gray-700 transition-all duration-200 font-medium shadow-lg hover:shadow-xl text-sm lg:text-base"
                            >
                                Order Again
                            </button>
                        )}
                    ...
                    <div className="p-4 border-b border-gray-200 bg-gray-50">
                        ...
                        >
                            <h3 className="font-semibold text-gray-800 group-hover:text-gray-700 transition-colors">Seguimiento del Pedido</h3>
                            ...
                            <div className="mt-4 space-y-3">
                                {statusRoadmap.map((step, index) => (
                                    <div key={step.status} className="flex items-center gap-3">
                                        ...
                                        <span className={`text-sm ${index <= currentStepIndex ? 'text-gray-800' : 'text-gray-500'}`}>
                                            {step.label}
                                        </span>
                                    </div>
                                ))}
                    ...
                    <div className="p-4">
                        <div className="flex items-center gap-3 mb-6">
                            ...
                            <div>
                                <p className="font-medium text-gray-800">�Necesitas ayuda con tu pedido?</p>
                                <p className="text-sm text-gray-500">Estoy aqu� para ti.</p>
                            </div>
                        </div>

                        <button className="w-full mb-6 bg-gray-600 text-gray-800 py-3 rounded-lg hover:bg-gray-700 flex items-center justify-center gap-2 font-medium transition-colors duration-200">
                            <MessageCircle className="w-4 h-4" />
                            Let's Chat
                        </button>
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="space-y-3">
                                <button className="flex items-center justify-between w-full text-left p-3 hover:bg-gray-50 rounded-lg border border-gray-200 transition-colors group">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            <p className="text-sm font-medium text-gray-800">Fiverr Pro FAQs</p>
                                ...
                                <button className="flex items-center justify-between w-full text-left p-3 hover:bg-gray-50 rounded-lg border border-gray-200 transition-colors group">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            <p className="text-sm font-medium text-gray-800">Resolution center</p>
                            ...
                            {isAdmin && search?.searchHire && (
                                <button
                                    ...
                                    className="w-full mt-3 bg-orange-500 text-gray-800 py-3 rounded-lg hover:bg-orange-600 text-sm font-medium transition-colors duration-200"
                                >
                                    Finalizar B�squeda
                                </button>
                            )}
                            ...
                            {canReview && (
                                <button
                                    onClick={() => setModalState((prev) => ({ ...prev, showReviewModal: true }))}
                                    className="w-full mt-3 bg-yellow-500 text-gray-800 py-3 rounded-lg hover:bg-yellow-600 text-sm font-medium transition-colors duration-200 flex items-center justify-center gap-2"
                                >
                                    <Star className="w-4 h-4" />
                                    Enviar Rese�a
                                </button>
                            ...
                            <div className="bg-gray-50 p-4 sm:p-6 border border-gray-200">
                                ...
                                {false ? ( // Ya no hay loading separado - viene en details-complete
                                    ...
                                    <div className="bg-red-50 border border-red-200 rounded-lg p-4">
                                        ...
                                        <button 
                                            onClick={() => window.location.reload()} // ✅ Recargar página para obtener datos actualizados
                                            className="px-4 py-2 bg-red-600 text-gray-800 text-sm rounded-lg hover:bg-red-700 transition-colors"
                                        >
                                            Reintentar
                                        </button>
                                        {false && ( // Ya no hay error separado - viene en details-complete
                                                        ...
                                                        <div className="flex items-center gap-2">
                                                            {deliverable.url.endsWith('.mp4') ? (
                                                                ...
                                                                <a 
                                                                    ...
                                                                    target="_blank" 
                                                                    rel="noopener noreferrer" 
                                                                    className="w-full sm:w-auto px-3 sm:px-4 py-2 bg-blue-600 text-gray-800 text-xs sm:text-sm rounded-lg hover:bg-blue-700 transition-colors flex items-center justify-center gap-2"
                                                                >
                                        ...
                                        <div className="space-y-3">
                                            ...
                                            <button
                                                ...
                                                className={`w-full py-2.5 sm:py-3 text-xs sm:text-sm rounded-lg flex items-center justify-center gap-2 font-medium transition-colors duration-200 ${selectedDeliverableFiles.length === 0 || isUploadingDeliverable
                                                    ...
                                                    : 'bg-gray-600 hover:bg-gray-700 text-gray-800'
                                                    }`}
                                                disabled={selectedDeliverableFiles.length === 0 || isUploadingDeliverable}
                                            >
                                                {isUploadingDeliverable ? (
            ...
            {process.env.NODE_ENV === 'development' && serviceInfo && (
                <div className="fixed top-16 right-2 z-50 bg-blue-100 border border-blue-300 rounded-lg p-2 text-xs max-w-xs lg:top-20 lg:right-4 lg:p-3">
                    <h4 className="font-bold text-blue-800 mb-1 text-xs">Debug:</h4>
                    <div className="space-y-0.5 text-blue-700 text-xs">
                        <div className="font-semibold text-blue-800 text-xs">Servicio:</div>
                        <div>ID: {serviceInfo?.serviceTypeId || 'N/A'}</div>
                    ...
                    <button
                        ...
                        className="mt-1 w-full bg-blue-600 text-gray-800 px-1 py-0.5 rounded text-xs hover:bg-blue-700"
                    >
                        Refresh
                    </button>
                    
                    {/* ✅ Debug de reseñas simplificado */}
                    <div className="mt-1 p-1 bg-yellow-100 border border-yellow-300 rounded text-xs">
                        <div className="font-bold text-yellow-800 mb-0.5 text-xs">Reseñas:</div>
                        <div>Estado: {search?.searchHire?.status || 'N/A'}</div>
                        <div>Reseña: {hasReviewed ? 'Sí' : 'No'}</div>
                        {review && <div>Score: {review.score}/5</div>}
                    </div>
                </div>
            )}
                        ...
                        <div className="flex justify-end space-x-3">
                            ...
                            {selectedDistributionStatus === 'appointment_cancelled_by_expert_rejection' && (
                                <button
                                    ...
                                    className="px-4 py-2 bg-red-600 text-gray-800 rounded-md hover:bg-red-700 transition-colors text-sm font-medium"
                                >
                            ...
                            {selectedDistributionStatus === 'appointment_cancelled_by_client_second' && (
                                <button
                                    ...
                                    className="px-4 py-2 bg-orange-600 text-gray-800 rounded-md hover:bg-orange-700 transition-colors text-sm font-medium"
                                >
                        ...
                        <div className="bg-gray-50 border-b border-gray-200 px-6 py-4">
                            <div className="flex items-center justify-between">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        <h2 className="text-xl font-semibold text-gray-800">Responder a la Disputa</h2>
                        ...
                        <div className="bg-gray-50 px-6 py-3 border-t border-gray-200">
                            <div className="flex gap-3 justify-end">
                                ...
                                <button
                                    ...
                                    className="px-4 py-2 bg-blue-600 text-gray-800 rounded-lg hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors text-sm font-medium flex items-center gap-2"
                                >
            ...
            {/* ✅ Reseña movida al tab Details - debug eliminado */}
        </div>
    );
}
