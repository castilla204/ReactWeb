...
function CoverThumb({ kind, custom }: { kind: DeliverableKind; custom?: boolean }) {
  if (kind === 'pdf') {
    return (
      <div className="relative flex h-[56px] w-[44px] shrink-0 flex-col gap-1 rounded-md border border-[#e2e2e2] bg-white p-2">
        <span className="h-[4px] w-[55%] rounded-sm bg-[hsl(var(--brand))]" />
        <span className="h-[3px] w-full rounded-sm bg-[#ededed]" />
        <span className="h-[3px] w-full rounded-sm bg-[#ededed]" />
        <span className="h-[3px] w-[70%] rounded-sm bg-[#ededed]" />
        {custom ? (
          <span
  ...
  return (
    <div className="flex h-[52px] w-[52px] shrink-0 items-center justify-center rounded-[10px] bg-[#eef2f6]">
      {icon}
    </div>
  );
}
...
export const ServiceDetailDeliverableCover: React.FC<ServiceDetailDeliverableCoverProps> = ({
  ...
  if (!selected) {
    return (
      <div className="overflow-hidden rounded-2xl border border-dashed border-[#e4e4e4] bg-[hsl(var(--ep-canvas))] opacity-80">
        <div className="flex items-center gap-3 px-4 py-3.5">
          ...
          <div className="min-w-0 flex-1">
            <p className="text-[13px] font-semibold text-[hsl(var(--ep-muted))]">{coverTitle}</p>
            <p className="text-[12px] text-[hsl(var(--ep-muted))]">{coverMeta}</p>
          </div>
  ...
  return (
    <button
      ...
      className="group block w-full overflow-hidden rounded-2xl border border-[#1c1c1c] bg-white text-left transition-colors"
    >
      <div className="flex items-center gap-3 border-b border-[#f0f0f0] bg-[#fafbfc] px-4 py-3.5">
        <CoverThumb kind={kind} custom={custom} />
        <div className="min-w-0 flex-1">
          <p className="mb-0.5 text-[11px] font-semibold uppercase tracking-[0.04em] text-[hsl(var(--brand))]">
            {coverTitle}
          </p>
          <p className="text-[12px] text-[#737373]">{coverMeta}</p>
        </div>
      </div>
      <div className="flex items-center justify-between gap-3 px-4 py-3">
        <span className="min-w-0 truncate text-[13px] text-[#1c1c1c]">{footerText}</span>
        <span className="inline-flex shrink-0 items-center gap-0.5 text-[13px] font-semibold text-[hsl(var(--brand))]">
          {linkText}
