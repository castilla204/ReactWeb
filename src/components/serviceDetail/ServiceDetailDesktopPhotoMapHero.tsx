...
export const ServiceDetailDesktopPhotoMapHero: React.FC<ServiceDetailDesktopPhotoMapHeroProps> = ({
  ...
  return (
    <div
      className={`relative grid grid-cols-1 items-stretch gap-0 lg:grid-cols-2 ${className}`}
    ...
    >
      <div className={`relative min-h-0 min-w-0 ${SD_DESKTOP_PHOTO_MAP_HERO_HEIGHT_CLASS}`}>
        <ServiceDetailDesktopGallery
          ...
          primaryOverlays={{
            ...
            bottom: titleOverlay ? (
              <div className="flex h-full flex-col justify-end bg-gradient-to-t from-black/40 via-black/10 to-transparent px-5 pb-4 pt-6">
      ...
      <div
        className={`relative min-h-0 min-w-0 overflow-hidden rounded-none border-l border-line ring-1 ring-inset ring-line ${SD_DESKTOP_PHOTO_MAP_HERO_HEIGHT_CLASS}`}
      >
        {location ? (
          <>
            ...
            {(locationLabel || radius || isWorkshopOnly) && (
              <div
                className="pointer-events-none absolute inset-x-0 bottom-0 z-[3] max-h-[38%] bg-gradient-to-t from-black/40 via-black/10 to-transparent px-3 pb-3 pt-6"
                aria-hidden
              >
          ...
          <div className="flex h-full flex-col items-center justify-center gap-2 bg-surface-tinted px-6 text-center">
            <MapPin className="h-8 w-8 text-ink-soft" strokeWidth={1.5} aria-hidden />
